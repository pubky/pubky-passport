...
function ConfirmationDialog({ actionLabel = "Confirm", children, description, destructive, onConfirm, title, triggerClassName, triggerLabel, triggerSize }: ConfirmationDialogProps) {
  ...
  return (
    <>
      <Button className={triggerClassName} onClick={open} size={triggerSize} type="button" variant={destructive ? "destructive" : "default"}>{triggerLabel}</Button>
      <dialog className="mb-0 mt-auto w-full max-w-[576px] rounded-t-xl border bg-popover p-6 text-foreground shadow-2xl backdrop:bg-black/75 open:flex open:flex-col open:gap-6 sm:m-auto sm:rounded-2xl sm:p-8" ref={ref}>
        <button aria-label="Close" className="absolute right-4 top-4 flex size-8 items-center justify-center rounded-full bg-secondary text-xl text-secondary-foreground" onClick={close} type="button">×</button>
        ...
        <footer className="flex flex-col gap-3 sm:flex-row sm:gap-4">
          <Button className="w-full sm:flex-1" onClick={close} type="button" variant="outline">Cancel</Button>
          <Button className="w-full sm:flex-1" onClick={() => { close(); onConfirm?.(); }} type="button" variant={destructive ? "destructive" : "default"}>{actionLabel}</Button>
        </footer>
      </dialog>
    </>
  );

...
function ConfirmationDialog({ actionLabel = "Confirm", children, description, destructive, onConfirm, title, triggerClassName, triggerLabel, triggerSize }: ConfirmationDialogProps) {
  ...
  return (
    <>
      ...
      <Dialog className="mb-0 mt-auto w-full max-w-[576px] rounded-t-xl border bg-popover p-6 text-foreground shadow-2xl backdrop:bg-black/75 open:flex open:flex-col open:gap-6 sm:m-auto sm:rounded-2xl sm:p-8" onOpenChange={setOpen} open={open}>

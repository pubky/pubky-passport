...
const JSON_TOKEN_VALUE_PATTERN = new RegExp(
  `(["'])\\b(${SENSITIVE_TOKEN_KEYS})\\b\\1\\s*:\\s*(["'])[^"']+\\3`,
  "giu",
);
const TOKEN_VALUE_PATTERN = new RegExp(
  `\\b(${SENSITIVE_TOKEN_KEYS})\\b\\s*[:=]\\s*([^\\s,;&'\"]+)`,
  "giu",
);
// A 32-byte secret encoded as base64url is exactly 43 characters. This covers
// the server-derived wrapping key (server/wrapping-key/google/deriveGoogleWrappingKey.ts) and the Pubky auth
...
const FAIL_CLOSED_OPAQUE_VALUE_PATTERN =
  /(?<![A-Za-z0-9+/_-])[A-Za-z0-9+/_-]{43,}={0,2}(?![A-Za-z0-9+/_=-])/gu;

function redactAuthorizationUrls(value: string): string {
  return value
    .replace(PUBKY_AUTH_URL_PATTERN, AUTHORIZATION_URL_REDACTION)
    .replace(
      RELATIVE_AUTHORIZE_QUERY_URL_PATTERN,
...
function redactSensitiveAndOpaqueValues(value: string): string {
  return value
    .replace(
      ...
      (_match, scheme: string) => `Authorization: ${scheme} ${TOKEN_REDACTION}`,
    )
    .replace(JWT_PATTERN, TOKEN_REDACTION)
    .replace(
      JSON_TOKEN_VALUE_PATTERN,
      (_match, keyQuote: string, key: string, valueQuote: string) =>
        `${keyQuote}${key}${keyQuote}:${valueQuote}${TOKEN_REDACTION}${valueQuote}`,
    )

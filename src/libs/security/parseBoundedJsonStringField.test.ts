...
import { parseBoundedJsonStringField } from "./parseBoundedJsonStringField";

describe("parseBoundedJsonStringField", () => {
  it("accepts an exact non-empty field with JSON parameters", async () => {
    const result = await parseBoundedJsonStringField(
...
function jsonRequest(body: unknown, contentType: string): Request {
  return new Request("https://passport.pubky.app/api/test", {
    method: "POST",

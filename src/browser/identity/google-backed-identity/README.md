...
`createGoogleBackedIdentityRuntime.ts` is the feature composition root. It wires
...
- `homegate-invitation/` owns the invitation contract and concrete browser adapter.
- `wrapping-key/` owns the browser contract and adapter for Passport's server API.
- `createGoogleBackedIdentityRuntime.ts` is the only file in this feature that
  constructs concrete dependencies.

Google sign-in and Drive consent remain in the parent identity feature because they
acquire credentials for multiple identity actions rather than establish an identity.

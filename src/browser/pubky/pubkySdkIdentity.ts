...
/**
 * Thin adapter around the concrete Pubky SDK facade. It accepts an SDK keypair
 * and does not know about browser key handles or feature contracts.
 */
export class PubkySdkIdentity {
  ...
  async approveAuthRequest(input: PubkyAuthApprovalInput): Promise<PubkyIdentityOperationResult<void>> {
    ...
    return this.withSigner(input.keypair, async (signer) => {
      await signer.approveAuthRequest(input.authRequest.sensitivePubkyAuthUrl);

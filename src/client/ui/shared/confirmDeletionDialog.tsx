    ...
    >
      <form className="flex flex-col gap-6" onSubmit={submit}>
        <div className="flex flex-col gap-2 pr-10">
          <h2 className="text-xl font-bold leading-7" id={titleId}>{title}</h2>
          {description ? <p className="text-sm leading-5 text-muted-foreground" id={descriptionId}>{description}</p> : null}
        </div>
        <IconButton aria-label="Close" className="absolute right-[15px] top-[15px]" disabled={pending} onClick={cancel} type="button" variant="secondary">
        ...
        <div className="flex flex-col gap-3">
          {retryAction ? (
            <Button className="w-full" disabled={pending} onClick={retryAction.onClick} size="lg" type="button" variant="outline">
              {retryAction.label}
            </Button>
          ) : null}

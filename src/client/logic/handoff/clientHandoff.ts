 ...
 */
export async function handoffClientMessage(
  appWindow: Window,
  callback: string,
  message: { type: string; acknowledgementType: string; payload: Record<string, unknown> },
  signal: AbortSignal,
): Promise<ClientHandoffStatus> {
...
function waitForAcknowledgement(
  ...
  const onMessage = (event: MessageEvent) => {
    if (
      ...
      isAcknowledgement(event.data, messageId, acknowledgementType)
    )
      settle(true);
  };
...
function navigationStatus(appWindow: Window, callback: string): ClientHandoffStatus {
  try {
...
function logHandoffFailure(operation: string, cause: unknown): void {
  LOGGER.warn("client.callback_handoff.failed", {
    operation,

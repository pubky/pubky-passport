...
describe("validateRelayUrl", () => {
  ...
  it("accepts the relay URL length limit and rejects limit plus one", () => {
    ...
    const atLimit = `${prefix}${"a".repeat(PUBKY_AUTH_URL_LIMITS.maximumRelayUrlCodeUnits - prefix.length)}`;
...
describe("validatePubkyAuthUrls", () => {
  ...
  it("accepts the callback URL length limit and rejects limit plus one", () => {
    ...
    const atLimit = `${prefix}${"a".repeat(PUBKY_AUTH_URL_LIMITS.maximumCallbackUrlCodeUnits - prefix.length)}`;
  ...
  it("allows missing callbacks", () => {
    const result = validatePubkyAuthUrls(
      authUrl("relay=https://httprelay.pubky.app/inbox&secret=secret-value"),
    );
  ...
  it("rejects other non-HTTPS callback schemes by default", () => {
    for (const callback of ["http://third.example/success", "pubky://third.example/success"]) {
      expectUrlError(
        authUrl(
          `relay=https://httprelay.pubky.app/inbox&secret=secret-value&x-success=${callback}`,
        ),
        "invalid_callback",
      );
    }

...
authorization UI. Pubky Auth request parsing and validation are colocated here;
identity persistence and concrete SDK key handling remain in their owning browser features.
...
- `parsePubkyAuthRequest.ts`, `parsePubkyAuthCapabilities.ts`, and
  `validatePubkyAuthUrls.ts` own the bounded browser authorization request grammar.
- `browserAuthorizationRequest.ts` converts validated request data into an

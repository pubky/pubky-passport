...
2. Parse it into safe review data plus an opaque validated request.
...
- `request/` owns the opaque validated request, while `request/parser/` groups the
  stateless untrusted-protocol validators it composes.
...
- `instrumentation-client.ts` captures and retains the pre-hydration entry until the
  controller takes it.
- `entry/authorizationEntry.ts` consumes and scrubs browser input, then constructs a validated request.
- `request/ValidatedPubkyAuthRequest.ts` owns safe review data and private request metadata.
...
The protocol parser cluster lives in `request/parser/` and is split into
`pubkyAuthRequestParser.ts`, `pubkyAuthCapabilities.ts`, and `pubkyAuthUrls.ts`.
Each validator owns the bounds it enforces. These modules are stateless and keep
protocol validation separate from browser and SDK lifecycles.

## Security Invariants

- The raw authorization URL, secret, and complete callbacks never enter UI state.
- Only the exact live `ValidatedPubkyAuthRequest` can retrieve approval data.
...
3. `request/ValidatedPubkyAuthRequest.ts`

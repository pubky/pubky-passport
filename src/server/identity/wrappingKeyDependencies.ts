...
import type { IdentityProviderId, VerifiedProviderIdentity } from "../../features/identity/identityProvider";

/**
 * Dependencies required only by the server-owned wrapping-key flow.
 * Concrete Google, clock, rate-limit, and HKDF code stays in server identity.
 */
export type Clock = {
  now(): Date;
};

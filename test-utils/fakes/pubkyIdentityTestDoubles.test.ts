...
describe("Pubky identity test doubles", () => {
  ...
  it("simulates discovery publication success and failure", async () => {
    ...
    await expectDiscoveryError(discovery.publishHomeserver({ keyHandle: key.keyHandle }), "publish_failed");
  });
  ...
  it("simulates auth approval without recording raw pubkyauth URLs", async () => {
    ...
    const parsedAuthRequest = IssuedPubkyAuthRequest.issue(encodeURIComponent(
    ...
    const authRequest = expectOk(parsedAuthRequest);
